...
<script type="text/javascript">
	...
	function selectUser4(id){
		...
		$("#selectUser4").removeAttr("href").attr("href",url).click();
	}
</script>
		...
		<c:if test="${type eq 'orderAlertPersionsStategy' || type eq 'provisionsAlertPersionsStategy' }">
			<c:forEach items="${configs}" var="con">
				<div class="row col-xs-10">	
					...
					<div class="col-xs-6"><textarea id="configVlue2_${con.configId }" name="configVlue2s" rows="" class="form-control input-sm" cols="80" readonly="readonly" onclick="selectUser4('${con.configId }');">${con.codevalue2 }</textarea></div>
					<label style="display: none">
						<a id="selectUser4" href="#" lookupGroup="usersLook" width="400" height="500"><i class="icon-search"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
					</label>
					<label>
						<a href="#" onclick="selectUser4('${con.configId }');"><i class="icon-search"></i> 选择</a>
					</label>
				</div>
			</c:forEach>	
		</c:if>
