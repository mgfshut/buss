...
<div class="searchBar container-fluid">
	<div class="row">
		<div class="col-xs-4">
			<ys:codemapSelect2 codemap="txStatus"  selectName="txStatus" value="${param.txStatus}" defaultText="交易状态"
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="140" style="margin-top: -50px;height: 300px;">
		<thead>
			<tr>
		...
		<tbody>
			<c:forEach items="${resList}" var="item">
				<tr target="transactionInfoId" rel="${item.transactionInfoId}">
					<td><a href="module/sys-transactionInfo-detail/transactionInfo-detail-${item.transactionInfoId }" 
						title="查看详情" target="navTab" rel="traneditmanager" data-parent="traneditmanager">${item.cateName}</a></td>
					<td>${item.manuNum}</td>
					<td>${item.prodPla}</td>
					<td>${item.cateStan}</td>
					<td>${item.cusName}</td>
