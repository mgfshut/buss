...
<script type="text/javascript">
	...
	function validForm3(){
		$("#configPagerForm3").submit();
	}
	
	function selectUser3(id){
		...
		$("#selectUser3").removeAttr("href").attr("href",url).click();
	}
</script>


<form id="configPagerForm3" role="form" action="service/config-save" method="post" onsubmit="return validateCallback(this,navTabAjaxDone)">
		...
		<c:if test="${type eq 'orderAlertPersionsStategy' || type eq 'provisionsAlertPersionsStategy' }">
			<c:forEach items="${configs}" var="con">
				<div class="row col-xs-10">	
					...
					<div class="col-xs-6"><textarea id="configVlue2_${con.configId }" name="configVlue2s" rows="" class="form-control input-sm" cols="80" readonly="readonly" onclick="selectUser3('${con.configId }');">${con.codevalue2 }</textarea></div>
					<label style="display: none">
						<a id="selectUser3" href="#" lookupGroup="usersLook" width="400" height="500"><i class="icon-search"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
					</label>
					<label>
						<a href="#" onclick="selectUser3('${con.configId }');"><i class="icon-search"></i> 选择</a>
					</label>
				</div>
			</c:forEach>	
		</c:if>
		<br>
		<div class="col-xs-offset-10 col-xs-1" style="text-align: right;"><button class="btn btn-sm btn-primary" type="button" onclick="validForm3();"><i class="icon-save"></i>  <span>保存</span></button></div>
	</div>
</div>
</div>
</form> 
