...
var DWZ = {
	...
	pageInfo: {currentPage:"currentPage", showCount:"showCount", orderField:"orderField", orderDirection:"orderDirection"},
...
(function($){
	...
	$.fn.extend({
		...
		loadUrl: function(url,data,callback){
			$(this).ajaxUrl({url:url, data:data, callback:callback,type:'POST'});
		},
		initUI: function(){
		...
		layoutMaxH: function($refBox){
			return this.each(function(){
				var $this = $(this);
				if (! $refBox) $refBox = $this.parents("div.layoutBox:first");
				var iRefH = $refBox.height();
				var iLayoutH = parseInt($this.attr("layoutMaxH"));
				var iH = iRefH - iLayoutH > 50 ? iRefH - iLayoutH : 50;
				console.log('--------------------')
				if ($this.isTag("table")) {
					$this.removeAttr("layoutMaxH").wrap('<div layoutMaxH="'+iLayoutH+'" style="overflow:auto;max-height:'+iH+'px"></div>');
				} else {
					$this.css({"overflow":"auto","max-height":iH});
				}
			});
		},
...
function initLayout(){
	...
	$("#container .tabsPageContent").height(iContentH - 48).find("[layoutH]").layoutH();
	$("#sidebar-nav,#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH - 35).find('[layoutH]').layoutH();
	$('#sidebar-nav [layoutMaxH]').layoutMaxH();
...
function initUI(_box){
	...
	//tables
	$("table.table:not(.bootstrap)", $p).jTable();
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external,parent:parent});
	...
	$("div.pagination", $p).each(function(){
		...
		$this.pagination({
			...
			rel:$this.attr("rel"),
			totalResult:$this.attr("totalResult"),
			showCount:$this.attr("showCount"),
			currentPageShown:$this.attr("currentPageShown"),
...
$.setRegional("alertMsg", {
	title:{error:"错误", info:"提醒", warn:"警告", correct:"成功", confirm:"确认"},
	butMsg:{ok:"确认", yes:"是", no:"否", cancel:"取消"}
});
var alertMsg = {
	_boxId: "#alertMsgBox",
...
var navTab = {
	...
	_closeTab: function(index, openTabid){
		var parentId = this._getTabs().eq(index).data('parent');
		this._getTabs().eq(index).remove();
		//this._getPanels().eq(index)
		...
		var openTabid = openTabid || parentId;
		if (openTabid) {
		...
		this._init();
		this._scrollCurrent();
		this._reload(this._getTabs().eq(this._currentIndex),openTabid);
	},
	_closeTabOnly:function(index){
		this._getTabs().eq(index).remove();
		this._getPanels().eq(index).trigger(DWZ.eventType.pageClear).remove();
		this._getMoreLi().eq(index).remove();
		if (this._currentIndex >= index) this._currentIndex--;
	...
	closeTabOnly:function(){
		if (this._currentIndex > 0) {this._closeTabOnly(this._currentIndex);}
	},
	...
	_loadUrlCallback: function($panel){
		...
		$panel.find(":button.close,[data-dismiss='modal']").click(function(){
			navTab.closeCurrentTab();
		});
		
		loadSelect2Style($panel);
		totalPattern($panel);
	},
...
(function($){
	$.pdialog = {
			...
			if (dialog){
				...
				jDContent.ajaxUrl({
					type:"POST", url:url, data:op.data, callback:function(response){
						...
						$(":button.closedialog", dialog).click(function(){
							$.pdialog.close(dialog);
							return false;
						});
						loadSelect2Style($(dialog));
						totalPattern($(dialog));
						if ($.isFunction(op.callback)) op.callback(response);
					}
				});
		...
		reloadForData:function(url, data, options){
			var op = $.extend({data:data, dialogId:"", callback:null}, options);
			var dialog = (op.dialogId && $("body").data(op.dialogId)) || this._current;
			if (dialog){
				var jDContent = dialog.find(".dialogContent");
				jDContent.ajaxUrl({
...
//					$.pdialog.switchDialog(dialog);
		...
		openDataDialog:function(url, dlgid, title, options, data) {
			var op = $.extend({},$.pdialog._op, options);
			var dialog = $("body").data(dlgid);
			//重复打开一个层
			if(dialog) {
...
function navTabSearch(form, navTabId){
	var $form = $(form);
	if (form[DWZ.pageInfo.currentPage]) form[DWZ.pageInfo.currentPage].value = 1;
...
/**
 * 
 * @param {Object} args {currentPage:"",showCount:"",orderField:"",orderDirection:""}
...
function _getPagerForm($parent, args) {
	...
	if (form) {
		if (args["currentPage"]) form[DWZ.pageInfo.currentPage].value = args["currentPage"];
		if (args["showCount"]) form[DWZ.pageInfo.showCount].value = args["showCount"];
 ...
 * data: pagerForm参数 {currentPage:"n", showCount:"n", orderField:"xxx", orderDirection:""}
 * callback: 加载完成回调函数
 */
function dwzPageBreak(options){
	var op = $.extend({ targetType:"navTab", rel:"", data:{currentPage:"", showCount:"", orderField:"", orderDirection:""}, callback:null}, options);
...
/**
 * 处理navTab中的分页和排序
 * @param args {currentPage:"n", showCount:"n", orderField:"xxx", orderDirection:""}
...
(function($){
	$.fn.extend({
		pagination: function(opts){
			var setting = {
				...
				currentPageFrag:'<li class="#liClass#"><a href="javascript:;">#currentPage#</a></li>'
			};
			return this.each(function(){
				var $this = $(this);
				...
				for (var i=interval.start; i<interval.end;i++){
					currentPageFrag += setting.currentPageFrag.replaceAll("#currentPage#", i).replaceAll("#liClass#", i==pc.getCurrentPage() ? 'selected j-num' : 'j-num');
				}
				
				$this.html(DWZ.frag["pagination"].replaceAll("#currentPageFrag#", currentPageFrag).replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
				...
				if (pc.hasPrev()){
					...
					_bindEvent($prev, pc.getCurrentPage()-1, pc.targetType(), pc.rel(),pc.opts.showCount);
					_bindEvent($first, 1, pc.targetType(), pc.rel(),pc.opts.showCount);
				} else {
				...
				if (pc.hasNext()) {
					...
					_bindEvent($next, pc.getCurrentPage()+1, pc.targetType(), pc.rel(),pc.opts.showCount);
					_bindEvent($last, pc.numPages(), pc.targetType(), pc.rel(),pc.opts.showCount);
				} else {
					$next.add($last).addClass("disabled").find(">a").hide();
				}
	
				$this.find(setting.nums$).each(function(i){
					_bindEvent($(this), i+interval.start, pc.targetType(), pc.rel(),pc.opts.showCount);
				});
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						var currentPage = $inputBox.val();
						if (currentPage && currentPage.isPositiveInteger()) {
							dwzPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {currentPage:currentPage}});
						}
					});
			...
			function _bindEvent($target, currentPage, targetType, rel,showCount){
				$target.bind("click", {currentPage:currentPage}, function(event){
					dwzPageBreak({targetType:targetType, rel:rel, data:{currentPage:event.data.currentPage,showCount:showCount}});
					event.preventDefault();
				});
			}
		},
	...
	$.extend(Pagination.prototype, {
		...
		numPages:function() {
			return Math.ceil(this.opts.totalResult/this.opts.showCount);
		},
		getInterval:function(){
			var ne_half = Math.ceil(this.opts.currentPageShown/2);
			var np = this.numPages();
			var upper_limit = np - this.opts.currentPageShown;
			var start = this.getCurrentPage() > ne_half ? Math.max( Math.min(this.getCurrentPage() - ne_half, upper_limit), 0 ) : 0;
			var end = this.getCurrentPage() > ne_half ? Math.min(this.getCurrentPage()+ne_half, np) : Math.min(this.opts.currentPageShown, np);
...
(function($){
	...
	$.extend({
		bringBackSuggest: function(args, target){
			var $box = _lookup['$target'].parents(".unitBox:first");
			$box.find(":input").each(function(){
				var $input = $(this);
				if (target){
					if (target == "dialog"){
						$input = $(this, $.pdialog.getCurrent());
					}else if (target == "navtab"){
						$input = $(this, navTab.getCurrentPanel());
					}
				}
		...
		bringBack: function(args, target){
			$.bringBackSuggest(args, target);
			$.pdialog.closeCurrent();
		}
	});
 ...
 * 将普通的选择框转变为select2样式
 */
function loadSelect2Style(openPanel){
	try{
		//当加载一个选项卡，自动为select的对象使用select2样式
		$(openPanel[0]).find(".select2").each(function(){
			var node = this.nodeName;
			if (node.toLowerCase() == "select"){
 ...
 * 数字计算通用方法
 */
function totalPattern(openPanel){
	try{
		$(openPanel[0]).find("input[totalPattern]").each(function(){
			var tPattern = $(this).attr("totalPattern");
			if (tPattern){
				$(this).change(function(){
					var pArray = tPattern.split(";");
					for (var p=0; p<pArray.length; p++){
						var pattern = pArray[p];
						try{
							//匹配参与计算的对象。最后一个对象为目标存储对象
							var pat = new RegExp("\\W");
							//匹配参与计算的运算符。运算符比对象少一个
							var fhPat = new RegExp("\\w*");
							var array = pattern.split(pat);
							var fhArray = pattern.replace(fhPat, "").split(fhPat);
							var tmp = 0;
							for (var i=0; i<array.length-1; ){
								var fh = fhArray[i % 2];
								var firstVal = parseFloat($("#"+array[i]).val());
								if (i == 0){
									tmp = parseFloat($("#"+array[i+1]).val());
								}
								var secVal = tmp;
								if (fh == "+"){
									tmp = firstVal + secVal; 
							...
							$("#"+array[array.length-1]).val(tmp.toFixed(2));
						}catch(e){
