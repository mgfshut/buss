...
function change(v){
	...
	if(v==3){
		...
		if(checkAddress == ''){
			document.getElementById("address").value=  $("#s1").find("option:selected").text()+$("#s2").find("option:selected").text()+$("#s3").find("option:selected").text();	
		}
	}
}
var dsy = new Dsy();
...
dsy.add("0",["北京","天津","上海","重庆","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西","海南省","四川省","贵州省","云南省","西藏","陕西省","甘肃省","青海省","宁夏","新疆","香港","澳门","台湾省"]);var s=["s1","s2","s3"];
var opt0 = ["省份","地级市","市、县级"];
function setup(){
